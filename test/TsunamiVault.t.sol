...
contract Unit_deposit is Deployed {
  ...
  function test_revertWhen_tokenIsAddressZero() public {
    ...
    vm.prank(ALICE.addr);
    vm.expectRevert();
    testContract.deposit(address(0), TARGET_AMOUNT);
  }

  function test_revertWhen_tokenIsNotValidERC20() public {
    _activate();
    vm.prank(ADMIN.addr);
    testContract.whitelistToken(BOB.addr, true);
    vm.prank(ALICE.addr);
    vm.expectRevert();
    testContract.deposit(BOB.addr, TARGET_AMOUNT);
  }
...
contract Fuzz_deposit is Deployed {
  function test_fuzzDepositAmount(uint256 amount) public {
    ...
    assertEq(
      testCoin.balanceOf(address(testContract)), ALICE_DEPOSIT + BOB_DEPOSIT + amount, 'invalid contract balance'
    );
    assertEq(
      testContract.userBalance(address(testCoin), ALICE.addr), ALICE_DEPOSIT + amount, 'invalid alice deposit recorded'
    );
    assertEq(
...
contract Unit_switchPause is Deployed {
  function test_revertWhen_callerNotOwner() public {
    vm.prank(EVE.addr);
    vm.expectRevert(abi.encodeWithSelector(IERC173.IERC173_NOT_OWNER.selector, EVE.addr));
    testContract.switchPause();
  }

  function test_switchPause_updateStateAsExpected() public {
    vm.startPrank(ADMIN.addr);
    assertEq(testContract.state(), testContract.INACTIVE(), 'invalid initial state');
    testContract.switchPause();
    assertEq(testContract.state(), testContract.ACTIVE(), 'invalid transitional state');
    testContract.switchPause();
    assertEq(testContract.state(), testContract.INACTIVE(), 'invalid final state');
    vm.stopPrank();
  }
}
...
contract Unit_whitelistToken is Deployed {
  ...
  function test_whitelistToken_updateWhitelistedToken() public {
    vm.startPrank(ADMIN.addr);
    assertEq(testContract.whitelistedToken(address(testCoin)), false, 'invalid initial whitelisting status');
    testContract.whitelistToken(address(testCoin), true);
    assertEq(testContract.whitelistedToken(address(testCoin)), true, 'invalid transitional whitelisting status');
    testContract.whitelistToken(address(testCoin), false);
    assertEq(testContract.whitelistedToken(address(testCoin)), false, 'invalid final whitelisting status');
    vm.stopPrank();
  }
}
// ****************
...
contract InitialDeposit is Deployed {
  function test_initialDeposit_isZero(address coin, address user) public {
    assertEq(testContract.userBalance(coin, user), 0, 'invalid initial deposit');
  }
}
// ****************
// **************************************
